...
<div id="tab-trace" class="tab-panel">
  ...
  <div id="traceResultsArea" style="display:none">
    ...
    <div class="chart-container" style="padding:10px 16px">
      ...
      <div style="display:flex;gap:4px;margin-top:8px" id="traceViewModeBtns">
        <button class="fileops-chip active" data-trace-mode="chain">📜 ${escHtml(t('trace.modeChain'))}</button>
        <button class="fileops-chip" data-trace-mode="list">📋 ${escHtml(t('trace.modeList'))}</button>
        <button class="fileops-chip" data-trace-mode="graph">🗺️ ${escHtml(t('trace.modeGraph'))}</button>
      </div>
    </div>
    <!-- Results content -->
  ...
  <div id="traceEmptyState" class="chart-container" style="text-align:center;padding:40px">
    ...
    <p style="color:var(--text-dim);font-size:13px">${escHtml(t('trace.emptyHint'))}</p>
...
var TRACE_SOURCE_TYPES = {
  user_input:     { icon: '👤', label: '${escHtml(t('trace.srcUserInput'))}',    color: '#3b82f6', badgeVar: 'blue'   },
  model_output:   { icon: '🤖', label: '${escHtml(t('trace.srcModelOutput'))}',   color: '#10b981', badgeVar: 'green'  },
  subagent_output:{ icon: '🤖', label: '${escHtml(t('trace.srcSubagentOutput'))}',color: '#10b981', badgeVar: 'green'  },
  tool_output:    { icon: '🔧', label: '${escHtml(t('trace.srcToolOutput'))}',    color: '#8b5cf6', badgeVar: 'purple' },
  root_dispatch:  { icon: '📤', label: '${escHtml(t('trace.srcRootDispatch'))}',  color: '#f97316', badgeVar: 'orange' },
};

var TRACE_MEDIUMS = {
  user_input:    { icon: '↓', label: '${escHtml(t('trace.medUserInput'))}' },
  tool_output:   { icon: '↓', label: '${escHtml(t('trace.medToolOutput'))}' },
  task_dispatch: { icon: '↓', label: '${escHtml(t('trace.medTaskDispatch'))}' },
  model_reasoning:{ icon: '↓', label: '${escHtml(t('trace.medModelReasoning'))}' },
};
...
function renderTraceRecentSearches() {
  ...
  var html = '<span style="font-size:10px;color:var(--text-dim)">${escHtml(t('trace.recentSearches'))}: </span>';
...
function renderTraceExamples() {
  ...
  var html = '<span style="font-size:10px;color:var(--text-dim)">${escHtml(t('trace.exampleHints'))}: </span>';
...
function renderTraceResultsHeader() {
  ...
  header.innerHTML = '<span style="font-size:14px;font-weight:600">${escHtml(t('trace.searchFor'))} &quot;' + esc(traceLastKeyword) + '&quot;</span>' +
    '<span class="fileops-op-badge ok" style="margin-left:8px;font-size:12px">' + traceSearchResults.length + ' ${escHtml(t('trace.hits'))}</span>';
...
function renderTraceChainView(container) {
  ...
  if (origin) {
    ...
    html += '<span style="font-size:11px;padding:2px 8px;border-radius:10px;' + obadgeStyle + ';font-weight:600">🟢 ${escHtml(t('trace.origin'))}</span>';
    ...
    html += '<button class="card-btn card-btn-sm" data-nav-turn="' + esc(origin.item.turnId) + '">${escHtml(t('trace.viewTurnBtn'))} →</button>';
    html += '</div>';
    html += '</div>';
  }
  ...
  for (var ci = 1; ci < chain.length; ci++) {
    ...
    if (isExpanded) {
      ...
      html += '<button class="card-btn card-btn-sm" data-nav-turn="' + esc(item.turnId) + '">${escHtml(t('trace.viewTurnBtn'))} →</button>';
      html += '</div>';
    }
    html += '</div>';
  }
...
function renderTraceGraphView(container) {
  ...
  if (nodes.length === 0) {
    container.innerHTML = '<div class="empty-state">${escHtml(t('trace.noNodes'))}</div>';
    return;
  }
  ...
  svg += '<text x="54" y="64" font-size="9" fill="#888" dominant-baseline="central">${escHtml(t('trace.legSeqFlow'))}</text>';
  if (hasBranch) {
    svg += '<line x1="110" y1="64" x2="142" y2="64" stroke="#f97316" stroke-width="2" stroke-dasharray="4 2"/>';
    svg += '<text x="146" y="64" font-size="9" fill="#f97316" dominant-baseline="central">${escHtml(t('trace.legDispatch'))}</text>';
  }
  ...
  for (var ni = 0; ni < nodes.length; ni++) {
    ...
    if (isOrigin) {
      svg += '<text x="' + (nx - r - 6) + '" y="' + ny + '" text-anchor="end" dominant-baseline="central" ' +
        'font-size="10" fill="#10b981" font-weight="bold">${escHtml(t('trace.origin'))} #' + node.turnIndex + '</text>';
    }
  }
