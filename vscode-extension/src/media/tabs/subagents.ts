...
export function renderSubagentsTab(): string {
  return `
<div id="tab-subagents" class="tab-panel">
  <div class="cards" id="subagentsCards"></div>
  <div class="table-wrap" style="margin-top:12px">
    <div class="table-header">${escHtml(t('subagents.title'))}</div>
    <div style="max-height: 500px; overflow-y: auto;">
      <table>
        <thead><tr>
          <th>${escHtml(t('subagents.colAgentName'))}</th>
          <th>${escHtml(t('subagents.colSessionId'))}</th>
          <th>${escHtml(t('subagents.colTurns'))}</th>
          <th>${escHtml(t('subagents.colTokens'))}</th>
          <th>${escHtml(t('subagents.colCost'))}</th>
          <th>${escHtml(t('subagents.colLatency'))}</th>
          <th>${escHtml(t('subagents.colModel'))}</th>
        </tr></thead>
...
export function renderSubagentsJS(): string {
  return `
// ── Subagents Tab ──
